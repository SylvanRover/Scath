...
if(shadowarea_sz > 0 || global.GLR_DIRECTIONAL_ENABLED  && global.GLR_DIRECTIONAL_STRENGTH > 0 )
{
    ...
    //draw_clear_alpha(global.GLR_AMBIENT_COLOR, color_get_value(global.GLR_AMBIENT_COLOR)/2);
    shader_set(glr_shader_directional_gamma);
    var r,g,b, col = global.GLR_AMBIENT_COLOR;
    r = color_get_red(col)/255;
    g = color_get_green(col)/255;
    b = color_get_blue(col)/255;
    
    var amb_int = 0.7;
    shader_set_uniform_f(global.GLR_UNIF_DIRECTIONAL_AMBIENT, r, g, b, amb_int);  
    //amb_int = 1 + amb_int * 3;  
    //var s =  (1-global.GLR_DIRECTIONAL_STRENGTH) * 255 * amb_int;
    var s = (1-global.GLR_DIRECTIONAL_STRENGTH) * 255;
    var col = make_colour_rgb(min(255, r*s), min(255, g*s), min(255, b*s));
...
for(var i = 0; i < ds_list_size(global.GLR_LIGHT_LIST); i++)
{
    ...
    matrix_set(matrix_world, matrix_multiply( l_id[| GLR_LIGHT.matrix], matrix_build(-v_x0 -0.1 , -v_y0-0.1, 0, 0, 0, 0, 1, 1, 1)));  
...
if(global.GLR_OCCLUSION_ENABLED)
{
    ...
    if(global.GLR_OCCLUSION_ENABLED && surface_exists(global.GLR_DEPTH_SURFACE)){
        ...
        draw_surface_stretched_ext(global.GLR_DEPTH_SURFACE, -0.01, -0.01, sur_w, sur_h,0,1); 
        shader_reset(); 
    } 
